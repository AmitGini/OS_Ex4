...
Graph::Graph(int vertices) : vertices(vertices), adjMatrix(vertices, std::vector<int>(vertices, 0)) {
    for(std::vector<int> &i : adjMatrix) {
        for(int &j : i) {
            j = 0;
        }
    }
...
// Destructor - set all values in adjacency matrix to 0 (for safety better coverage results)
Graph::~Graph() {
    for(std::vector<int> &i : adjMatrix) {
        for(int &j : i) {
            j = 0;
...
void Graph::eulerianCycle(){   
    // Check if the graph answer the Eulerian conditions
    if (!isEulerian()) { 
        std::cout << "Graph is not Eulerian\n";
        return;
    }

    std::vector<std::vector<int>> tempMatrix = adjMatrix;  // Step 1: Make a copy of the adjacency matrix
    std::stack<int> curr_path;  // Step 2: Stack to store the current path
    std::vector<int> circuit;  // Step 2: Vector to store the Eulerian Circuit

    curr_path.push(0);  // Step 3: Start from vertex 0
    int curr_v = 0;  // Step 3: Current vertex
   ...
   // Step 4: While the stack is not empty
    ...
    // Step 5: Print the Eulerian Circuit in reverse order.
